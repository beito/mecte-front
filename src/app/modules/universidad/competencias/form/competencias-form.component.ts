...
@Component({
  ...
  styleUrls: ['./competencias-form.component.scss']
})
export class CompetenciasFormComponent {
  @Input() competencias: CompetenciasClass;
  @Input() method: "insert" | "update" | "show" = "insert";
  @Input() data!: CompetenciasInterface;
  @ViewChild('form_ref', { static: false }) form_ref: FormRenderComponent;

  public competenciaID: number = 0;
  ...
  ngOnInit() {
    if (this.data.id && this.data.id > 0) {
      this.competenciaID = this.data.id;
    }
    this.competencias = new CompetenciasClass(this.data);
  ...
  customSubmit = (form: CompetenciasClass) => {
    this.swalAlertDriver();
    let competencia = form.raw();
    delete competencia.id;
    if (this.competenciaID) {
      this.competenciasApi.updateCompetencia(this.competenciaID, competencia).subscribe((data: {}) => {
      ...
      this.competenciasApi.createCompetencia(competencia).subscribe((data: {}) => {
        this.swalAlertDriver(1);
        this.modalService.dismissAll(true);
      }, (err) => {
        this.swalAlertDriver(2);
  ...
  swalAlertDriver(status: number = 0) {
    if (status === 1) {
      swal.hideLoading();
      swal.update({ title: 'Completado', html: 'Competencia registrada correctamente', icon: 'success' });
    } else if (status === 2) {
      swal.hideLoading();
  ...
  formConfigs() {
    this.form_config = {
      ...
      submit: () => {
        this.customSubmit(this.competencias);
      },
      rows: [
        {
          class: 'row',
          ...
          cols: [
            {
              ...
              buttons: [
                ...
                {
                  class: `btn btn-sm ${(this.competenciaID > 0) ? 'btn-warning': 'btn-success'} ms-2`,
                  label: (this.competenciaID > 0) ? 'Actualizar': 'Guardar',
                  type: 'submit',
                  icon: {
                    name: '',
                    class: 'fa fa-check',
